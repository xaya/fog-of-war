...
attribution. On the honest path the referee performs no curve arithmetic ---
its only in-round work is refusing a published set that does not rebuild the
committed root, which is hashing --- and the chain is touched only to open,
dispute and settle.
\end{abstract}
...
position and action, salted by a per-match secret $\seed_i$. The same move
...
depends on: every root is public before any flight exists on the wire.
...
(\S\ref{sec:audit}) --- with one bounded exception, an over-reported set,
...
and the correctness of the exchange's \emph{content} is never judged before a
dispute. The honest path therefore never touches the chain --- a channel
consults it only to open, dispute and settle. Three facts combine into the
enforcement story.
...
hundred-odd curve derivations, orders of magnitude beyond its per-call budget
--- so each check works from supplied or published bytes instead. On the
accused's side that is one Merkle-authenticated element of the committed set,
recomputed and verified in a handful of hashes. On the disputer's side a single
...
the stake must exceed the value of the information --- and conviction runs only
inside a dispute's adjudication, so the bound applies to fabrications that must
surface in one. At more than two participants
a dispute is also terminal for the whole table rather than for the pair: the
audit it forces reveals every participant's seed, and revealed seeds cannot
continue a hidden-information game, so the match settles on its merits --- the
same property the demand-an-immediate-end move has by design.
